...
for pair_id_csv in pair_ids:
    open(configuration['DIRECTORIES']['qsrnet_dir'] + '/data_output/' + 'metric_qsr_' + pair_id_csv + '.csv', 'w')
    ...
    while True:
        ...
        for pair_id in pair_ids:
            ...
            if metrics['velocity1'] != {}:
                with open(configuration['DIRECTORIES']['qsrnet_dir'] + '/data_output/' + 'metric_qsr_' + pair_id + '.csv', 'a') as f:
                    field_name = ['pcd distance', 'center distance', 'velocity 1', 'velocity 2', 'velocity 3', 'rcc', 'qtc1', 'qtc2', 'qtc3', 'qdc', 'probability']
                    metric_qsr_dict = {}
                    metric_qsr_dict['pcd distance'] = metrics['pcd_distances'][pair_id]; metric_qsr_dict['center distance'] = metrics['center_distances'][pair_id]; metric_qsr_dict['velocity 1'] = metrics['velocity1'][pair_id]
                    metric_qsr_dict['velocity 2'] = metrics['velocity2'][pair_id]; metric_qsr_dict['velocity 3'] = metrics['velocity3'][pair_id]
                    metric_qsr_dict['rcc'] = qsr_joint_max_state['state']['rcc']; metric_qsr_dict['qtc1'] = qsr_joint_max_state['state']['qtc1']; metric_qsr_dict['qtc2'] = qsr_joint_max_state['state']['qtc2']
                    metric_qsr_dict['qtc3'] = qsr_joint_max_state['state']['qtc3']; metric_qsr_dict['qdc'] = qsr_joint_max_state['state']['qdc']; metric_qsr_dict['probability'] = qsr_joint_max_state['probability']
