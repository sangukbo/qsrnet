...
def compute_metrics_from_point_clouds(pcd_dict, object_ids, timestamp, previous_center_dict):
    pcd_distance_dict = pcd_distances(pcd_dict, object_ids)
    center_dict = compute_centers(pcd_dict, object_ids)
    center_distance_dict = compute_center_distances(center_dict, object_ids)
    velocity1_dict, velocity2_dict, velocity3_dict, timestamp = compute_velocity(center_dict, previous_center_dict, object_ids, timestamp)
...
def pcd_distances(pcd_dict, object_ids):
    pcd_distance_dict = {}
    for m in range(len(object_ids)):
        for n in range(m+1, len(object_ids)):
...
def compute_center_distances(center_dict, object_ids):
...
def compute_velocity(center_dict, previous_center_dict, object_ids, timestamp):
